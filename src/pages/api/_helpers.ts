// ============================================================
// Shared helpers for Astro API Routes (Cloudflare D1 / Workers)
// ============================================================
import { env as cfEnv } from "cloudflare:workers";

const _env = cfEnv as unknown as {
  DB: D1Database;
  JWT_SECRET: string;
};
...
export async function requireAuth(request: Request): Promise<Record<string, unknown> | null> {
  const cookieHeader = request.headers.get('Cookie') || '';
  const token = parseCookie(cookieHeader, 'auth_token');
  if (!token) return null;
  const payload = await verifyJWT(token, _env.JWT_SECRET);
  return payload; // null if invalid
}

export async function requireAdmin(request: Request): Promise<Record<string, unknown> | null> {
  const cookieHeader = request.headers.get('Cookie') || '';
